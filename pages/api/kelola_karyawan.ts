...
export const config = {
  api: {
    bodyParser: false, // Menggunakan formidable untuk form parsing
  },
};

export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { id } = req.query; // Mengambil ID dari URL

  // Logging untuk melacak request method dan id
  ...
  if (req.method === 'POST') {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      // Validasi: hanya username, fullName, password, dan role yang wajib
      ...
      try {
        ...
        let photoUrl = '/daiku/profile.png'; // Default photo
        ...
        const newEmployee = await prisma.employee.create({
          data: {
            ...
            email: email ? String(email) : null,  // Email opsional
            department: department ? String(department) : null,  // Department opsional
            password: hashedPassword,
            whatsapp: whatsapp ? String(whatsapp) : null,  // WhatsApp opsional
            photo: photoUrl,
            role: String(role),
          },
        });
  ...
  // Operasi DELETE untuk menghapus karyawan (dengan id)
